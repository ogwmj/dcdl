...
const ICON_ADD = '➕';
/** @type {string} Icon for updating an item. */
const ICON_UPDATE = '🔄';
/** @type {string} Icon for editing an item. */
const ICON_EDIT = '✏️';
/** @type {string} Icon for deleting an item. */
const ICON_DELETE = '🗑️';
/** @type {string} Icon for calculation. */
const ICON_CALCULATE = '⚙️';
/** @type {string} Icon for saving. */
const ICON_SAVE = '💾';
/** @type {string} Icon for cancelling an action. */
const ICON_CANCEL = '❌';
/** @type {string} Icon for swapping items. */
const ICON_SWAP = '🔁';
/** @type {string} Icon for resetting. */
const ICON_RESET = '↩️';
/** @type {string} Icon for pre-filling. */
const ICON_PREFILL = '✨';
/** @type {string} Icon for exporting data. */
const ICON_EXPORT = '📤';
/** @type {string} Icon for importing data. */
const ICON_IMPORT = '📥';
/** @type {string} Icon for confirming an action. */
const ICON_CONFIRM = '✔️';
/** @type {string} Icon for sharing. */
const ICON_SHARE = '🔗';
/** @type {string} Icon for unsharing. */
const ICON_UNSHARE = '🚫';
/** @type {string} Icon for copying to clipboard. */
const ICON_COPY = '📋';

/** @type {string} The application ID, defaults if not provided by the environment. */
...
/** @type {FirebaseConfig} Default Firebase configuration if not provided by environment. */
...
/** @type {FirebaseConfig} Firebase configuration, uses environment if available. */
const firebaseConfig = typeof __firebase_config !== 'undefined' ? JSON.parse(__firebase_config) : firebaseConfigProvided;

/** @type {string|null} Initial authentication token if provided by environment. */
...
let auth;
/** @type {import("firebase/firestore").Firestore} Firebase Firestore instance. */
let db;
/** @type {import("firebase/analytics").Analytics} Firebase Analytics instance. */
let analytics;
/** @type {string|null} Current authenticated user's ID. */
let userId = null;
/** @type {any} DataTables instance for the roster. */
let rosterDataTable = null;
/** @type {boolean} Visibility state of the score column in the roster table. */
let scoreColumnVisible = true;
/** @type {Function|null} Callback function for the team name modal. */
let teamNameModalCallback = null;
/** @type {Function|null} Callback function for the confirm modal's confirm action. */
let confirmModalConfirmCallback = null;
/** @type {Function|null} Callback function for the confirm modal's cancel action. */
let confirmModalCancelCallback = null;
/** @type {Object|null} Stores the original best team calculated before any modifications. */
let originalBestTeam = null;
/** @type {Object|null} Stores the currently displayed team, which might be modified from the original. */
let currentDisplayedTeam = null;
/** @type {number} Index of the champion to be replaced in the swap modal. */
let championToReplaceIndex = -1;
/** @type {Array<Object>} Array holding synergy definitions fetched from the database. */
let dbSynergies = [];
/** @type {Array<Object>} Array holding champion definitions fetched from the database. */
let dbChampions = [];
/** @type {Array<Object>} Array holding legacy piece definitions fetched from the database. */
let dbLegacyPieces = [];
/** @type {Array<Object>} Array holding the player's current champion roster. */
let playerChampionRoster = [];
/** @type {string|null} Class of the currently selected champion in the form. */
let currentSelectedChampionClass = null;
/** @type {string|number|null} ID of the champion currently being edited. */
let editingChampionId = null;
/** @type {Object|null} The best team calculated, stored for saving purposes. */
let currentBestTeamForSaving = null;
/** @type {Array<Object>} Array holding the player's saved teams. */
let savedTeams = [];
...
/** @type {HTMLElement} Modal element for swapping champions. Created dynamically. */
...
/**
 * Star and color tier definitions with corresponding multipliers.
...
/** @type {number} Points awarded per star increment for legacy pieces. */
const LEGACY_PIECE_POINTS_PER_STAR_INCREMENT = 5;
/**
 * Scores for legacy piece star tiers.
 * @type {Object<string, number>}
 */
const LEGACY_PIECE_STAR_TIER_SCORE = {};

/**
 * Generates scores for legacy piece star tiers based on colors and star counts.
 */
...
generateLegacyPieceStarTierScores(); // Initialize the scores

/**
 * Base scores for champion rarities.
 * @type {Object<string, number>}
 */
...
/**
 * Base scores for legacy piece rarities.
 * @type {Object<string, number>}
 */
...
/** @type {number} Multiplier for class diversity bonus. */
const CLASS_DIVERSITY_MULTIPLIER = 1.15;
/** @type {number} Number of champions required to activate a synergy. */
const SYNERGY_ACTIVATION_COUNT = 3;
/** @type {number} Multiplier for synergy cohesion bonus. */
const SYNERGY_COHESION_MULTIPLER = 5;
/** @type {number} Base value for synergy cohesion bonus. */
...
/**
 * Displays an error message in the designated error indicator area and as a toast.
...
function showError(message, details = "") {
    ...
    if (errorIndicatorEl) {
        ...
        const p = errorIndicatorEl.querySelector('p');
        if (p) p.textContent = message;
    }
    if (errorMessageDetailsEl) errorMessageDetailsEl.textContent = details;
    showToast(message, 'error');
}

/**
 * Generates HTML for star rating display.
 * @param {string} starColorTier - The star/color tier string (e.g., "Gold 3-Star").
...
/**
 * Generates HTML for a healer icon placeholder.
...
/**
 * Generates HTML for a class icon placeholder.
...
/**
 * Generates an array of team combinations from a given array of champions.
 * @param {Array<Object>} array - The array of champions to generate combinations from.
 * @param {number} k - The size of each combination (team size).
 * @returns {Array<Array<Object>>} An array of team combinations.
 */
...
/**
 * Initializes Firebase app, auth, db, and analytics. Handles user authentication.
 * @returns {Promise<void>} A promise that resolves when Firebase is initialized and user is authenticated.
 * @throws {Error} If Firebase initialization or authentication fails.
 */
async function initializeFirebase() {
    try {
        ...
        setLogLevel('error'); // Set Firestore log level
                    ...
                    try {
                        if (initialAuthToken) {
                            ...
                            console.log("Signed in with custom token.");
                             userId = auth.currentUser?.uid; // Re-check userId
                             if (userId && userIdDisplay) userIdDisplay.textContent = `User ID: ${userId.substring(0,8)}...`;
                        } else {
                            await signInAnonymously(auth);
                            console.log("Signed in anonymously.");
                             userId = auth.currentUser?.uid; // Re-check userId
                             if (userId && userIdDisplay) userIdDisplay.textContent = `User ID: ${userId.substring(0,8)}... (Anonymous)`;
                        }
                        resolve();
                    } catch (error) {
                        console.error("Error during sign-in:", error);
                        showError("Firebase Authentication failed: " + error.message);
                        if (userIdDisplay) userIdDisplay.textContent = "User ID: Firebase auth error";
                        reject(error);
                    }
                }
            }, (error) => {
...
/**
 * Fetches synergy definitions from Firestore and renders them.
 * @async
 */
async function fetchSynergiesAndRender() {
    if (!db) {
        showError("Firestore database is not initialized. Cannot fetch synergies.");
        return;
    }
    try {
        ...
        dbSynergies = [];
        renderAvailableSynergies(); // Render empty state
    }
}

/**
 * Fetches champion definitions from Firestore.
 * @async
 */
async function fetchChampions() {
    if (!db) {
        showError("Firestore database is not initialized. Cannot fetch champions.");
        return;
    }
    try {
        ...
        dbChampions = querySnapshot.docs.map(doc => {
            ...
            return {
                id: doc.id,
                ...data,
                isHealer: data.isHealer === true // Ensure boolean
            };
        });
    } catch (error) {
...
/**
 * Fetches legacy piece definitions from Firestore and populates the select dropdown.
 * @async
 */
async function fetchLegacyPieces() {
    if (!db) {
        showError("Firestore database is not initialized. Cannot fetch legacy pieces.");
        return;
    }
    try {
...
async function loadPlayerRosterFromFirestore() {
    if (!userId || !db) {
        console.warn("Cannot load roster: User not authenticated or DB not initialized.");
        ...
        if (docSnap.exists()) {
            ...
            if (rosterData && Array.isArray(rosterData.champions)) {
                playerChampionRoster = rosterData.champions.map(savedChamp => {
                    ...
                    return {
                        ...savedChamp,
                        // Ensure 'id' is present and correctly typed (it's Date.now() + Math.random(), so a number)
                        id: savedChamp.id || (Date.now() + Math.random()), // Fallback if ID was missing
...
/**
 * Loads saved teams from Firestore for the current user.
 * @async
 */
async function loadSavedTeams() {
    ...
    try {
        ...
        if (savedTeamsListEl) savedTeamsListEl.innerHTML = `<p class="text-red-500">Error loading saved teams.</p>`;
    }
}
 ...
 * @param {Object<string, number>} tiersObject - An object containing tier names and their values.
...
/**
 * Populates gear rarity select dropdowns with standard gear rarities.
 */
...
function renderAvailableSynergies() {
    ...
    dbSynergies.forEach(synergyDef => {
        ...
        // Added cursor-pointer to the header
        synergyItemHeader.className = 'synergy-item-header flex items-center justify-between hover:bg-slate-100 p-2 rounded-md -m-2 mb-1 cursor-pointer';
        ...
        collapseIndicator.className = 'collapse-indicator text-lg font-bold ml-auto mr-2'; // Added ml-auto to push to right
        collapseIndicator.textContent = '+'; // Default to collapsed
        ...
        `;
        // Insert collapse indicator before the progress span
        const progressSpan = synergyItemHeader.querySelector('.synergy-progress');
        if (progressSpan) {
            synergyItemHeader.insertBefore(collapseIndicator, progressSpan);
        } else {
            synergyItemHeader.appendChild(collapseIndicator); // Fallback if progress span isn't there
        }
        ...
        // Create a container for the content that will be collapsed/expanded
        const synergyContentDiv = document.createElement('div');
        synergyContentDiv.className = 'synergy-content hidden'; // Initially hidden
        ...
        if (contributingChampions.length > 0) {
            ...
            synergyContentDiv.appendChild(championsListDiv);
        } else {
            const noChampsP = document.createElement('p');
            noChampsP.className = 'text-xs text-slate-500 mt-1 pl-4 no-champs-text'; // Added class for targeting
            noChampsP.textContent = 'No champions in your roster have this synergy yet.';
            synergyContentDiv.appendChild(noChampsP);
        }
        synergyItemContainer.appendChild(synergyContentDiv); // Append the collapsible content

        // Add click listener to the header for collapsing
        synergyItemHeader.addEventListener('click', () => {
            const content = synergyItemContainer.querySelector('.synergy-content');
            const indicator = synergyItemHeader.querySelector('.collapse-indicator');
            if (content) {
             ...
             if (analytics) logEvent(analytics, 'toggle_synergy_details', { synergy_name: synergyDef.name, is_collapsed: content ? content.classList.contains('hidden') : true });
        });
...
/**
 * Populates the champion select dropdown with available champions.
 * Filters out champions already in the roster unless editing.
 */
function populateChampionSelect() {
    ...
    if (availableChampions.some(champ => champ.id === currentSelectedValue)) {
    ...
    } else if (!editingChampionId) { // Reset dependent fields if selection is cleared and not editing
        ...
        if (champInherentSynergiesDisplayEl) champInherentSynergiesDisplayEl.textContent = 'Select a base champion to see synergies.';
    }
 ...
 * @param {string|null} [championClass=null] - The class of the champion to filter legacy pieces for.
 */
function populateLegacyPieceSelect(championClass = null) {
    ...
    if (championClass && championClass !== "N/A") {
        ...
        filteredLegacyPieces = dbLegacyPieces.filter(lp => {
            ...
            // Show if description is empty (universal) OR includes the champion's class
            return description === "" || description.includes(lowerChampionClass);
        });
    } else { // If no champion class, only show universal legacy pieces
        filteredLegacyPieces = dbLegacyPieces.filter(lp => {
            const description = (lp.description || "").toLowerCase();
            return description === "";
        });
...
function renderPlayerChampionRoster() {
    ...
    if (playerChampionRoster.length === 0) {
        ...
        playerChampionRoster.forEach(champ => {
            ...
            // Ensure champ.id is correctly passed as a string in onclick, but will be converted to number in handler
            tr.innerHTML = `
        ...
        if (typeof $ !== 'undefined' && $.fn.DataTable) {
            rosterDataTable = new $('#rosterTable').DataTable({
                responsive: true,
                "columnDefs": [
                    { "targets": [0, 1, 2, 4, 5, 6], "type": "string" }, // String sort for most
                    { "targets": 3, "type": "num", "className": "dt-column-score text-right" } // Numeric sort for score
                ],
                "order": [[3, "desc"]] // Default sort by score descending
            });
    ...
    renderAvailableSynergies(); // Update synergy counts
}
    ...
    savedTeams.forEach(team => {
        ...
        if (team.members && Array.isArray(team.members)) {
            team.members.forEach(member => {
                ...
                if (individualScore === undefined) { // Fallback if score wasn't saved with member
                    const baseChampDetails = dbChampions.find(c => c.id === member.dbChampionId) || {};
                    const memberForScoreCalc = { // Reconstruct enough data for score calculation
                        ...member,
                        baseRarity: member.baseRarity || baseChampDetails.baseRarity,
                        legacyPiece: { // Ensure legacyPiece structure
                            ...(member.legacyPiece || {}),
                ...
                memberCardHtml += `           ${getClassPlaceholder(member.class, 'result-icon class-icon mr-2')}`;
                ...
                if (member.inherentSynergies && member.inherentSynergies.length > 0) {
                    ...
                    member.inherentSynergies.forEach(synergy => {
                        ...
                        memberCardHtml += `<span class="icon-wrapper"><img src="img/factions/${synergyNameForIcon}.png" alt="${synergy}" title="${synergy}" class="result-icon w-5 h-5" onerror="this.style.display='none'; const fb = this.parentElement.querySelector('.icon-placeholder'); if (fb) fb.style.display='inline-block';"><span class="icon-placeholder text-xs" style="display:none;">[${synergy}]</span></span>`;
                    });
    ...
    if (teamObject.members && Array.isArray(teamObject.members)) {
        teamObject.members.forEach((member) => {
            ...
            if (member.inherentSynergies && member.inherentSynergies.length > 0) {
                ...
                member.inherentSynergies.forEach(synergy => {
                    const synergyNameForIcon = synergy.trim().replace(/\s+/g, '_');
                    html += `<span class="icon-wrapper"><img src="img/factions/${synergyNameForIcon}.png" alt="${synergy}" title="${synergy}" class="result-icon w-5 h-5" onerror="this.style.display='none'; const fb = this.parentElement.querySelector('.icon-placeholder'); if (fb) fb.style.display='inline-block';"><span class="icon-placeholder text-xs" style="display:none;">[${synergy}]</span></span>`;
                });
                html += `       </div></div>`;
            }
            html += `   </div>`;
 ...
 * @param {Object} teamToDisplay - The team object to display.
 */
function displayResults(teamToDisplay) {
    if (!resultsOutputEl) return;
    ...
    originalBestTeam = JSON.parse(JSON.stringify(teamToDisplay)); // Deep copy
    currentDisplayedTeam = JSON.parse(JSON.stringify(teamToDisplay)); // Deep copy
    currentBestTeamForSaving = JSON.parse(JSON.stringify(teamToDisplay)); // Deep copy

    renderTeamDisplay(currentDisplayedTeam, true); // Render with modifiable flag true
}

/**
 * Renders a team object into the results output area.
 * @param {Object} teamObject - The team data to render.
 * @param {boolean} [isModifiable=true] - Whether the displayed team can be modified (e.g., swap champions).
 */
...
/**
 * Saves the current player roster to Firestore.
 * @async
 */
async function savePlayerRosterToFirestore() {
    if (!userId || !db) {
        console.error("Cannot save roster: User not authenticated or DB not initialized.");
        showToast("Error: User not authenticated. Cannot save roster.", "error");
        return;
    }
    if (saveRosterIndicatorEl) saveRosterIndicatorEl.classList.remove('hidden');
    if (addUpdateChampionBtn) addUpdateChampionBtn.disabled = true;
    ...
    try {
        ...
        if (saveRosterIndicatorEl) saveRosterIndicatorEl.classList.add('hidden');
        if (addUpdateChampionBtn) addUpdateChampionBtn.disabled = false;
    }
}
...
function resetChampionForm() {
    ...
    if (champInherentSynergiesDisplayEl) champInherentSynergiesDisplayEl.textContent = 'Select a base champion to see synergies.';
    Object.values(gearSelectEls).forEach(sel => { if (sel) sel.value = STANDARD_GEAR_RARITIES[0]; });
    if (legacyPieceSelectEl) legacyPieceSelectEl.value = "";
    populateLegacyPieceSelect(null); // Populate with universal LPs
...
/**
 * Populates the champion form with data of a champion to be edited.
 * Exposed to global scope for inline HTML `onclick`.
 * @param {string|number} championIdParam - The ID of the champion to edit (comes as string from HTML).
 */
window.editChampion = (championIdParam) => {
    const championId = parseFloat(championIdParam); // Convert string ID from HTML to number
    ...
    if (!championToEdit) {
        ...
        showToast(`Error: Champion with ID ${championId} not found for editing.`, "error");
        return;
    }
    editingChampionId = championId; // Store as number
    if (formModeTitleEl) formModeTitleEl.textContent = "Edit Champion in Roster";

    if (champSelectDbEl) {
        champSelectDbEl.value = championToEdit.dbChampionId;
        champSelectDbEl.disabled = true; // Prevent changing base champion during edit
    }
    if (champBaseRarityDisplayEl) champBaseRarityDisplayEl.value = championToEdit.baseRarity;
    if (champClassDisplayEl) champClassDisplayEl.value = championToEdit.class || "N/A";
    if (champHealerStatusDisplayEl) champHealerStatusDisplayEl.value = championToEdit.isHealer ? 'Yes' : 'No';
    currentSelectedChampionClass = championToEdit.class || null;
    if (champStarColorEl) champStarColorEl.value = championToEdit.starColorTier;
    if (legacyPieceStarColorEl) legacyPieceStarColorEl.value = (championToEdit.legacyPiece && championToEdit.legacyPiece.starColorTier) ? championToEdit.legacyPiece.starColorTier : "Unlocked";
    if (champInherentSynergiesDisplayEl) champInherentSynergiesDisplayEl.textContent = (championToEdit.inherentSynergies || []).join(', ') || 'None';

    if (gearSelectEls.head) gearSelectEls.head.value = championToEdit.gear.head.rarity;
    if (gearSelectEls.arms) gearSelectEls.arms.value = championToEdit.gear.arms.rarity;
    if (gearSelectEls.legs) gearSelectEls.legs.value = championToEdit.gear.legs.rarity;
    if (gearSelectEls.chest) gearSelectEls.chest.value = championToEdit.gear.chest.rarity;
    if (gearSelectEls.waist) gearSelectEls.waist.value = championToEdit.gear.waist.rarity;

    populateLegacyPieceSelect(currentSelectedChampionClass); // Filter LPs based on current champ's class
    if (legacyPieceSelectEl) legacyPieceSelectEl.value = (championToEdit.legacyPiece && championToEdit.legacyPiece.id) ? championToEdit.legacyPiece.id : "";
    ...
    if (champSelectDbEl) window.scrollTo({ top: champSelectDbEl.offsetTop - 125, behavior: 'smooth' });
...
/**
 * Cancels the champion edit mode and resets the form.
 */
...
/**
 * Removes a champion from the player's roster.
 * Exposed to global scope for inline HTML `onclick`.
 * @param {string|number} championIdParam - The ID of the champion to remove (comes as string from HTML).
 * @async
 */
window.removePlayerChampion = async (championIdParam) => {
    const championId = parseFloat(championIdParam); // Convert string ID from HTML to number
    if (editingChampionId === championId) { // If deleting the champion currently being edited
        cancelEditMode();
    }
    ...
    if (champToRemove) {
        openConfirmModal(
            ...
            async () => {
                ...
                populateChampionSelect(); // Update available champions in dropdown
            }
        );
    } else {
        showToast(`Error: Champion with ID ${championId} not found for removal.`, "error");
        console.error("Champion to remove not found in roster:", championId);
    }
}
...
/**
 * Calculates the individual score of a champion based on rarity, stars, gear, and legacy piece.
 * @param {Object} champion - The champion object.
...
function calculateIndividualChampionScore(champion) {
    ...
    if (champion.starColorTier in STAR_COLOR_TIERS) {
        ...
        starMultiplier = 1.0; // Default if tier is unknown
...
/**
 * Recalculates the total score of a team, including synergy and diversity bonuses.
 * @param {Array<Object>} teamMembersArray - An array of champion objects in the team.
 * @returns {Object} The recalculated team object with updated scores and breakdown.
 */
function recalculateTeamScore(teamMembersArray) {
    ...
    // Sort synergies: percentage bonuses first, then others
    const sortedSynergyDefs = [...dbSynergies].sort((a, b) => {
        if (a.bonusType === 'percentage' && b.bonusType !== 'percentage') return -1;
        if (a.bonusType !== 'percentage' && b.bonusType === 'percentage') return 1;
        return 0; // Maintain original order for non-percentage or same-type
    });
    ...
    sortedSynergyDefs.forEach(synergyDef => {
        ...
        if (memberCount >= SYNERGY_ACTIVATION_COUNT) {
            ...
            // Apply primary bonus (percentage or flat)
            if (synergyDef.bonusValue !== undefined && synergyDef.bonusValue !== null) {
                if (synergyDef.bonusType === 'percentage') {
                    ...
                    scoreAfterPercentageSynergies += bonusValue; // Update base for subsequent percentage bonuses
                    primaryBonusAppliedThisSynergy = synergyDef.bonusValue; // Store the percentage
                } else if (synergyDef.bonusType === 'flat') {
                    accumulatedBaseFlatBonus += synergyDef.bonusValue;
                    primaryBonusAppliedThisSynergy = synergyDef.bonusValue; // Store the flat value
                }
            }
            // Apply tiered flat bonus based on member count
            if (memberCount === 3) tieredFlatBonusAppliedThisSynergy = SYNERGY_COHESION_BASE * SYNERGY_COHESION_MULTIPLER;
            else if (memberCount === 4) tieredFlatBonusAppliedThisSynergy = SYNERGY_COHESION_BASE * SYNERGY_COHESION_MULTIPLER * 2.5; // Example: 2.5x for 4 members
            else if (memberCount >= 5) tieredFlatBonusAppliedThisSynergy = SYNERGY_COHESION_BASE * SYNERGY_COHESION_MULTIPLER * 5;   // Example: 5x for 5+ members
    ...
    if (uniqueClassesInTeam.size >= 4) { // Assuming 4 unique classes for bonus
    ...
    // No analytics log here as this is a helper function, logged in the caller.
...
/**
 * Saves the current best team to Firestore after prompting for a team name.
 * @async
 */
async function saveCurrentBestTeam() {
    ...
    if (!currentDisplayedTeam) { // Use currentDisplayedTeam as it might have been modified
    ...
    openTeamNameModal(defaultTeamName, 'Save Team As', async (teamNameToSave) => {
        const teamDataToSave = {
            name: teamNameToSave,
            members: currentDisplayedTeam.members.map(m => ({ // Save detailed member info
                dbChampionId: m.dbChampionId,
                name: m.name,
                baseRarity: m.baseRarity,
                class: m.class,
                isHealer: m.isHealer === true, // Ensure boolean
                starColorTier: m.starColorTier,
                gear: m.gear, // Save full gear objects
                legacyPiece: { // Save full legacy piece object
                    id: m.legacyPiece.id,
                ...
                individualScore: m.individualScore // Save calculated individual score
            })),
            totalScore: currentDisplayedTeam.totalScore,
            activeSynergies: currentDisplayedTeam.activeSynergies, // Save active synergies
            scoreBreakdown: currentDisplayedTeam.scoreBreakdown, // Save score breakdown
            baseScoreSum: currentDisplayedTeam.baseScoreSum, // Save base score sum
        ...
        const saveTeamBtnEl = document.getElementById('save-team-btn');
        if(saveTeamBtnEl) saveTeamBtnEl.disabled = true;
        ...
        try {
            ...
            loadSavedTeams(); // Refresh the list of saved teams
        } catch (error) {
            console.error("Error saving team:", error);
            showToast("Failed to save team. Error: " + error.message, "error");
        } finally {
            if(saveTeamBtnEl) saveTeamBtnEl.disabled = false;
        }
    });
}

/**
 * Renames a saved team in Firestore.
 * Exposed to global scope for inline HTML `onclick`.
...
/**
 * Deletes a saved team from Firestore. If the team is shared, its public record is also deleted.
 * Exposed to global scope for inline HTML `onclick`.
...
window.deleteSavedTeam = async (teamId) => {
    ...
    openConfirmModal(
        ...
        async () => {
            try {
                let wasShared = false;
                // If the team has a publicShareId, delete the public document first
                ...
                // Delete the private team document
...
/**
 * Shares a saved team by creating a public record and updating the private record with the share ID.
 * Exposed to global scope for inline HTML `onclick`.
 * @param {string} teamId - The ID of the team to share.
 * @async
 */
window.shareTeam = async (teamId) => {
    ...
    openConfirmModal(
        `Are you sure you want to generate a public share link for the team "${teamToShare.name}"? Anyone with the link will be able to view it.`,
        async () => {
            try {
                const publicTeamData = {
                    name: teamToShare.name,
                    members: teamToShare.members.map(m => ({ // Ensure all necessary fields are copied
                        dbChampionId: m.dbChampionId,
                        name: m.name,
                        baseRarity: m.baseRarity,
                        class: m.class,
                    ...
                    createdAt: serverTimestamp(), // Timestamp for the public record
                    originalOwnerId: userId // Store original owner for reference if needed
                };
                ...
                // Update the private team document with the publicShareId
                const privateTeamDocRef = doc(db, `artifacts/${appId}/users/${userId}/savedTeams`, teamId);
...
/**
 * Removes the public share link for a team.
 * Exposed to global scope for inline HTML `onclick`.
 * @param {string} savedTeamId - The ID of the private saved team.
 * @param {string} publicShareId - The ID of the public shared team document.
 * @async
 */
window.unshareTeam = async (savedTeamId, publicShareId) => {
    if (!userId || !db || !publicShareId || !savedTeamId) {
    ...
    openConfirmModal(
        ...
        async () => {
            try {
                ...
                // Remove the publicShareId field from the private document
                ...
                loadSavedTeams(); // Refresh to show share button again
        ...
        null, // No specific cancel action needed beyond closing modal
        "Confirm Unshare"
    );
};
...
/**
 * Opens the processing modal with a status message and progress.
 */
 ...
 * @param {string} statusText - The text to display for the status.
...
/**
 * Opens the team name modal.
 * @param {string} [currentName=''] - The current name to pre-fill in the input.
 * @param {string} [title='Enter Team Name'] - The title for the modal.
 * @param {Function} callback - The function to call when the name is saved.
 */
function openTeamNameModal(currentName = '', title = 'Enter Team Name', callback) {
    if (!teamNameModalEl || !teamNameModalTitleEl || !teamNameInputEl) return;
...
/**
 * Closes the team name modal.
 */
function closeTeamNameModal() {
    if (!teamNameModalEl || !teamNameInputEl) return;
 ...
 * @param {Function|null} [onCancel=null] - Callback function if the user cancels.
...
/**
 * Opens the share team modal with the provided link.
 * @param {string} shareLink - The link to display in the modal.
 */
...
/**
 * Opens the swap champion modal, populating it with available champions from the roster.
 * Exposed to global scope for inline HTML `onclick`.
...
window.handleOpenSwapModal = (indexToReplace) => {
    ...
    if (!swapChampionModalEl.querySelector('.modal-content')) {
        ...
        `;
        const closeBtn = swapChampionModalEl.querySelector('#close-swap-modal-btn');
        if (closeBtn) {
        ...
        swapChampionModalEl.addEventListener('click', (event) => { // Close on backdrop click
    ...
    if (!swapModalBody || !currentDisplayedTeam) return;
    swapModalBody.innerHTML = ''; // Clear previous content
    ...
    if (availableToSwap.length === 0) {
        ...
        availableToSwap.forEach(champ => {
            ...
            li.dataset.champId = champ.id; // Use roster ID for selection
            ...
            li.addEventListener('click', () => {
                handleChampionSwap(champ.id, championToReplaceIndex); // Pass roster ID
                swapChampionModalEl.classList.add('hidden');
                swapChampionModalEl.classList.remove('active');
            });
            ul.appendChild(li);
    ...
    champSelectDbEl.addEventListener('change', (event) => {
        ...
        if (selectedChampionId) {
            ...
            if (selectedDbChampion) {
                if (champBaseRarityDisplayEl) champBaseRarityDisplayEl.value = selectedDbChampion.baseRarity || 'N/A';
                if (champClassDisplayEl) champClassDisplayEl.value = selectedDbChampion.class || 'N/A';
                if (champHealerStatusDisplayEl) champHealerStatusDisplayEl.value = selectedDbChampion.isHealer ? 'Yes' : 'No';
                currentSelectedChampionClass = selectedDbChampion.class || null;
                if (champInherentSynergiesDisplayEl) champInherentSynergiesDisplayEl.textContent = (selectedDbChampion.inherentSynergies || []).join(', ') || 'None';
            } else { // Should not happen if dbChampions is populated correctly
                currentSelectedChampionClass = null;
                if (champHealerStatusDisplayEl) champHealerStatusDisplayEl.value = '';
            }
        ...
        populateLegacyPieceSelect(currentSelectedChampionClass); // Update LPs based on class
    });
}
...
if (addUpdateChampionBtn) {
    addUpdateChampionBtn.addEventListener('click', async () => {
        ...
        } else { // If no legacy piece is selected, ensure star tier is "Unlocked" and score is 0
            legacyPieceData.starColorTier = "Unlocked"; // Default for no LP
            legacyPieceData.score = LEGACY_PIECE_BASE_RARITY_SCORE["None"] || 0; // Score for "None" rarity
        }

        if (editingChampionId) { // Update existing champion
            const championIndex = playerChampionRoster.findIndex(c => c.id === editingChampionId); // editingChampionId is already a number
            if (championIndex === -1) {
            ...
            playerChampionRoster[championIndex] = {
                ...playerChampionRoster[championIndex], // Keep existing ID, dbChampionId, name, etc.
                isHealer: baseChampionDataForUpdate ? (baseChampionDataForUpdate.isHealer === true) : (playerChampionRoster[championIndex].isHealer === true), // Re-confirm healer status
                starColorTier: champStarColorEl.value,
                gear: { // Update gear rarities and scores
                ...
                legacyPiece: legacyPieceData, // Update legacy piece data
            };
            renderPlayerChampionRoster();
            await savePlayerRosterToFirestore();
            ...
            cancelEditMode(); // Reset form and exit edit mode
            ...
            const playerChampion = {
                id: Date.now() + Math.random(), // Unique ID for roster management
                dbChampionId: baseChampionData.id, // ID from database
                name: baseChampionData.name,
                baseRarity: baseChampionData.baseRarity,
                class: baseChampionData.class || "N/A",
                isHealer: baseChampionData.isHealer === true, // Ensure boolean
            ...
            resetChampionForm();
            populateChampionSelect(); // Refresh dropdown to remove added champion
        }
    });
}
...
if (calculateBtn) {
    calculateBtn.addEventListener('click', async () => { // Make the event listener async
        ...
        if (playerChampionRoster.length < 5) {
            if (resultsOutputEl) resultsOutputEl.innerHTML = '<p class="text-red-500">You need at least 5 champions in your roster to form a team.</p>';
            return;
        }
        if (dbSynergies.length === 0 && resultsOutputEl) {
            showToast("Warning: No synergies loaded. Calculation will proceed without synergy bonuses.", "warning");
        }
        ...
        if (championsToExcludeIds.size > 0) {
            ...
            if (rosterForCombination.length < 5) {
                ...
                if (resultsOutputEl) resultsOutputEl.innerHTML = '<p class="text-red-500">After excluding champions, there are not enough remaining champions to form a team of 5.</p>';
        ...
        if (requireHealer) {
            ...
            if (healers.length === 0) {
                ...
                if (resultsOutputEl) resultsOutputEl.innerHTML = '<p class="text-red-500">No healers found to meet the "Require Healer" criteria.</p>';
                setTimeout(closeProcessingModal, 1500);
                return;
            }
            if (rosterForCombination.length < 5) {
                updateProcessingStatus("Error: Not enough champions for a team with a healer.", 100);
                if (resultsOutputEl) resultsOutputEl.innerHTML = `<p class="text-red-500">Not enough champions to form a team of 5 with a healer.</p>`;
                setTimeout(closeProcessingModal, 1500);
                return;
            }
            ...
            if (teamCombinations.length === 0 && healers.length > 0) {
                ...
                if (resultsOutputEl) resultsOutputEl.innerHTML = '<p class="text-red-500">Not enough other champions to form a valid team of 5 with any available healer.</p>';
                setTimeout(closeProcessingModal, 1500);
                return;
            }
        } else {
        ...
        if (teamCombinations.length === 0) {
            ...
            if (resultsOutputEl) resultsOutputEl.innerHTML = '<p class="text-red-500">Could not generate any valid teams with the current criteria.</p>';
            setTimeout(closeProcessingModal, 1500);
            return;
        }
        ...
        const BATCH_SIZE = 100; // Process 100 combinations per batch
        let currentIndex = 0;

        async function processBatch() {
            const batchEndTime = Date.now() + 16; // Aim for roughly 60fps (16ms per frame)
            while (Date.now() < batchEndTime && currentIndex < totalCombinations) {
                const team = teamCombinations[currentIndex];
                const evaluatedTeam = recalculateTeamScore(team);
                if (evaluatedTeam.totalScore > maxScore) {
                    maxScore = evaluatedTeam.totalScore;
                    bestTeam = evaluatedTeam;
                }
                currentIndex++;
            }

            const evaluationProgress = Math.round((currentIndex / totalCombinations) * 75);
            const currentOverallProgress = 20 + evaluationProgress;
            updateProcessingStatus(`Evaluating team ${currentIndex} of ${totalCombinations}...`, currentOverallProgress);
        ...
        requestAnimationFrame(processBatch); // Start the first batch
    });
}
...
if (saveTeamNameBtn) {
    saveTeamNameBtn.addEventListener('click', () => {
        ...
        if (teamNameModalCallback) {
            teamNameModalCallback(teamName); // Execute the callback with the name
        }
        closeTeamNameModal();
    });
}
...
if (teamNameModalEl) { // Close on backdrop click
...
if (excludeSavedTeamCheckboxEl) {
    excludeSavedTeamCheckboxEl.addEventListener('change', () => {
        if (selectExclusionTeamDropdownEl) {
            selectExclusionTeamDropdownEl.disabled = !excludeSavedTeamCheckboxEl.checked;
            if (!excludeSavedTeamCheckboxEl.checked) { // Clear selections if disabled
...
if (prefillRosterBtn) {
    prefillRosterBtn.addEventListener('click', async () => {
        const prefillAction = async () => {
            ...
            playerChampionRoster = []; // Clear existing roster
            dbChampions.forEach(baseChamp => {
                const defaultGear = {};
                // Initialize all gear slots to "None"
                ...
                const newChamp = {
                    ...
                    starColorTier: "Unlocked", // Default tier
                    gear: defaultGear,
                    legacyPiece: { // Default empty legacy piece
                        id: null, name: "None", rarity: "None", score: 0, starColorTier: "Unlocked", description: ""
                    },
                };
            ...
            renderPlayerChampionRoster();
            await savePlayerRosterToFirestore();
            populateChampionSelect(); // Refresh select options
        ...
        } else { // No current roster, confirm pre-fill directly
            openConfirmModal(
...
if (exportRosterBtn) {
    exportRosterBtn.addEventListener('click', () => {
        ...
        // Export only necessary data for re-import
        const exportableRoster = playerChampionRoster.map(champ => {
            return {
                dbChampionId: champ.dbChampionId, // Reference to base champion
                starColorTier: champ.starColorTier,
                gear: { // Only gear rarities
                ...
                legacyPiece: { // Only legacy piece ID and its star tier
...
if (importRosterBtn) {
    importRosterBtn.addEventListener('click', () => {
        if (importRosterFileEl) importRosterFileEl.click(); // Trigger file input
    });
}
...
if (importRosterFileEl) {
    importRosterFileEl.addEventListener('change', async (event) => {
        ...
        if (file.type !== "application/json") {
            ...
            importRosterFileEl.value = ''; // Reset file input
            return;
        }
            ...
            const processImport = async () => {
                try {
                    ...
                    for (const importedChamp of importedData) {
                        ...
                        const playerChampion = {
                            id: Date.now() + Math.random(), // New unique ID for this roster instance
                            ...
                            gear: { // Reconstruct gear with scores
                    ...
                    renderPlayerChampionRoster();
                    await savePlayerRosterToFirestore();
                    populateChampionSelect(); // Refresh dropdown
                    if (importErrors > 0) {
...
if (confirmModalEl) { // Close on backdrop click
    confirmModalEl.addEventListener('click', (event) => {
        if (event.target === confirmModalEl) {
            if (confirmModalCancelCallback) { // Also trigger cancel callback if defined
                confirmModalCancelCallback();
            }
            closeConfirmModal();
        }
...
if (shareTeamModalEl) { // Close on backdrop click
...
if (copyShareLinkBtn) {
    copyShareLinkBtn.addEventListener('click', () => {
        ...
        try {
            ...
            if (analytics) logEvent(analytics, 'share_link_copied', { link: shareTeamLinkInputEl.value });
        } catch (err) {
...
/**
 * Handles swapping a champion in the currently displayed team and recalculating the score.
 * Exposed to global scope for inline HTML `onclick`.
 * @param {string|number} selectedRosterChampIdParam - The ID of the champion from the roster to swap in (comes as string from HTML).
 * @param {number} indexToReplace - The index in the current team to replace.
 * @async
 */
window.handleChampionSwap = async (selectedRosterChampIdParam, indexToReplace) => {
    const selectedRosterChampId = parseFloat(selectedRosterChampIdParam); // Convert to number
    const newChampion = playerChampionRoster.find(rc => rc.id === selectedRosterChampId);

    if (!newChampion || !currentDisplayedTeam || indexToReplace < 0 || indexToReplace >= currentDisplayedTeam.members.length) {
        showToast("Error selecting champion for swap.", "error");
        console.error("Swap error: New champion or current team invalid.", {newChampion, currentDisplayedTeam, indexToReplace});
        return;
    }

    const newTeamMembers = [...currentDisplayedTeam.members];
    newTeamMembers[indexToReplace] = { // Replace with full new champion data
        ...newChampion,
        individualScore: calculateIndividualChampionScore(newChampion) // Ensure score is up-to-date
    };
    ...
    setTimeout(async () => {
        ...
        currentDisplayedTeam = recalculatedTeam; // Update the displayed team
        currentBestTeamForSaving = JSON.parse(JSON.stringify(recalculatedTeam)); // Update team for saving

        updateProcessingStatus("Updating display...", 90);
        renderTeamDisplay(currentDisplayedTeam, true); // Re-render the team display

        updateProcessingStatus("Recalculation complete!", 100);
        setTimeout(closeProcessingModal, 500);
    }, 50);

    if (analytics && newChampion && currentDisplayedTeam.members[indexToReplace]) {
        logEvent(analytics, 'execute_champion_swap', {
            swapped_in_champion_name: newChampion.name,
            swapped_out_champion_name: currentDisplayedTeam.members[indexToReplace].name // Name of the champ that was replaced
        });
    }
}

/**
 * Resets the displayed team to the original best calculated team.
 * Exposed to global scope for inline HTML `onclick`.
 */
window.handleResetTeam = () => {
    if (originalBestTeam) {
        ...
        renderTeamDisplay(currentDisplayedTeam, true); // Re-render, still modifiable
        showToast("Team reset to original calculation.", "info");
        if (analytics) logEvent(analytics, 'reset_displayed_team');
    }
}
...
async function main() {
    try {
        ...
        if (document.getElementById('champions-section')) { // Guard for teams.html elements
            ...
            if (saveTeamNameBtn) saveTeamNameBtn.innerHTML = `<span class="btn-icon">${ICON_SAVE}</span> <span class="btn-text">Save Name</span>`;
            if (cancelTeamNameBtn) cancelTeamNameBtn.innerHTML = `<span class="btn-icon">${ICON_CANCEL}</span> <span class="btn-text">Cancel</span>`;
            ...
            // Populate dynamic dropdowns and reset form
        ...
        if (loadingIndicatorEl) loadingIndicatorEl.classList.add('hidden');
        // Check if error was already shown by a sub-function
